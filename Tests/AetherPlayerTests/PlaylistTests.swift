...
final class PlaylistTests: XCTestCase {
    ...
    func testFilterAndNaturalSort() {
        let input = [u("ep10.mkv"), u("readme.txt"), u("ep2.mp4"), u("ep1.MKV"), u("feature.matroska"), u("disc.iso"), u("cover.jpg")]
        let result = playableFiles(in: input)
        XCTAssertEqual(result.map { $0.lastPathComponent }, ["disc.iso", "ep1.MKV", "ep2.mp4", "ep10.mkv", "feature.matroska"])
    }
    ...
    func testMediaPolicyCoversMatroskaAudioAndDiscImages() {
        XCTAssertTrue(MediaTypePolicy.isPlayable(u("movie.mkv")))
        XCTAssertTrue(MediaTypePolicy.isPlayable(u("movie.matroska")))
        XCTAssertTrue(MediaTypePolicy.isPlayable(u("album.flac")))
        XCTAssertTrue(MediaTypePolicy.isPlayable(u("archive.iso")))
        XCTAssertFalse(MediaTypePolicy.isPlayable(u("notes.txt")))
    }

    func testMediaPolicyUsesExplicitMatroskaTypeInAllPickers() {
        let identifiers = Set(MediaTypePolicy.playableContentTypes.map(\.identifier))
        XCTAssertTrue(identifiers.contains("org.matroska.mkv"))
        XCTAssertTrue(identifiers.contains("public.audio"))
        XCTAssertTrue(identifiers.contains("public.iso-image"))
    }
    ...
    func testManualPlaylistPreservesSelectedOrder() {
        let selected = [u("episode3.mkv"), u("episode1.mkv"), u("episode2.mkv")]
        let p = Playlist(items: selected, currentIndex: 0)

        XCTAssertEqual(p.items, selected)
        XCTAssertEqual(p.current, selected[0])
    }
}
