...
final class ScopedResource {
    ...
    /// Holds the security scope granted directly by a macOS open panel,
    /// Finder open, or drag-and-drop operation for the active playback.
    /// The caller may still use the URL when it is not security scoped (for
    /// example, a file inside the app container), in which case this returns
    /// nil and no matching stop call is needed.
    init?(url: URL) {
