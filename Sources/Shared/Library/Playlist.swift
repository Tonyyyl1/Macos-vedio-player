...
/// Compatibility aliases for existing call sites and tests. New code should
/// use `MediaTypePolicy` directly.
let videoExtensions = MediaTypePolicy.videoExtensions
let audioExtensions = MediaTypePolicy.audioExtensions
let playableExtensions = MediaTypePolicy.playableExtensions
...
func playableFiles(in urls: [URL]) -> [URL] {
    urls
        .filter(MediaTypePolicy.isPlayable)
